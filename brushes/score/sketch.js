export default function createBrush(engine) {
  ...
  let u = 9;              // base unit for staff spacing and marks
  let bs = 0.75;          // glob size multiplier
  let staffSize = 1;      // staff spacing and line weight, independent of globs
  ...
  function draw() {
    ...
    if (showStaff) engine.image(staffLayer, 0, 0);
    engine.image(paint, 0, 0);
    for (const s of active) renderSnippet(s, engine, now);
  }
  ...
  function advanceStroke() {
    ...
    for (let k = 0; k < 6; k++) {
      ...
      if (d < 0.4) continue;
      const step = engine.min(d, engine.max(1.2, d * SMOOTH_B));
      strokeTo(st.x + dx / d * step, st.y + dy / d * step);
    }
    if (!st.pressed && engine.dist(st.x, st.y, st.tx, st.ty) < 0.8) endStroke();
  }
  ...
  function strokeTo(x, y) {
    ...
    if (st.dx === 0 && st.dy === 0) { st.dx = dx / d; st.dy = dy / d; }
      ...
      const prev = engine.atan2(st.dy, st.dx);
      st.dx = engine.lerp(st.dx, dx / d, SMOOTH_HEADING);
      st.dy = engine.lerp(st.dy, dy / d, SMOOTH_HEADING);
      const m = engine.sqrt(st.dx * st.dx + st.dy * st.dy) || 1;
      st.dx /= m; st.dy /= m;
      dAng = engine.atan2(st.dy, st.dx) - prev;
      if (dAng > engine.PI) dAng -= engine.TWO_PI;
      if (dAng < -engine.PI) dAng += engine.TWO_PI;
    }
    const ang = engine.atan2(st.dy, st.dx);

    // a fast turn rolls the ribbon over like a twisted strip; otherwise it
    // relaxes to the nearest flat orientation
    if (engine.abs(dAng) / d > TWIST_THRESHOLD) {
      st.roll += dAng * TWIST_GAIN;
    } else {
      const flat = engine.round(st.roll / engine.PI) * engine.PI;
      st.roll = engine.lerp(st.roll, flat, engine.min(1, d * TWIST_RELAX));
    }
    const squash = engine.cos(st.roll); // projected width of the ribbon

    // extend the five staff lines
    const nx = -st.dy, ny = st.dx;
    const ends = [];
    const gap = staffGap();
    for (let k = -2; k <= 2; k++) ends.push([x + nx * k * gap * squash, y + ny * k * gap * squash]);
    if (st.ends) {
      for (let k = 0; k < 5; k++) {
    ...
    if (engine.abs(squash) < 0.8) st.travel = engine.min(st.travel, spacing);
    else {
      while (st.travel >= spacing) {
        st.travel -= spacing;
        const li = pickLineIndex();
        stamp(x + nx * li * gap, y + ny * li * gap, ang, li);
      }
    }
    st.x = x; st.y = y;
  }
  ...
  function endStroke() {
    if (st && st.len < 4) {
      ...
      const gap = staffGap();
      const x0 = st.x - gap * 4.5, x1 = st.x + gap * 4.5;
      for (let k = -2; k <= 2; k++) ribbonSegment(x0, st.y + k * gap, x1, st.y + k * gap);
      stamp(st.x, st.y, 0, 0);
    }
    st = null;
  }
  ...
  function ribbonSegment(x1, y1, x2, y2) {
    const weight = 0.8 * staffSize;
    if (autoDelay > 0) { ribbonQueue.push([engine.millis() + autoDelay, x1, y1, x2, y2, weight]); return; }
    staffLayer.stroke(0, 150);
    staffLayer.strokeWeight(weight);
  ...
  function flushRibbonQueue(now) {
    ...
    for (const q of ribbonQueue) {
      if (q[0] <= now) {
        staffLayer.strokeWeight(q[5]);
        staffLayer.line(q[1], q[2], q[3], q[4]);
      }
      else rest.push(q);
    }
    ribbonQueue = rest;
  ...
  function pickLineIndex() {
    ...
    if (r < 0.7) return engine.floor(engine.random(-2, 3));
    return engine.floor(engine.random(-4, 5));
  }
  ...
  function stamp(x, y, ang, li) {
    ...
    const col = pickColor();
    const dir = engine.random() < 0.6 ? -1 : 1; // mostly back over the ribbon just drawn
    const L = { li, dir, col };
    rec = [];
    switch (kind) {
  ...
  function schedule(prims, x, y, ang) {
    const speed = GROW_SPEED * engine.random(0.75, 1.25);
    let end = 0;
    for (const p of prims) {
      if (p.k === 'dot') {
        p.start = engine.dist(0, 0, p.x, p.y) / speed + engine.random(0, 60);
        p.dur = DOT_MS * engine.random(0.8, 1.3);
      } else if (p.k === 'path') {
        const a = p.pts[0], b = p.pts[p.pts.length - 1];
        if (engine.dist(0, 0, b[0], b[1]) < engine.dist(0, 0, a[0], a[1])) {
          p.pts.reverse();
          const w = p.wAt;
          p.wAt = t => w(1 - t);
        }
        p.len = pathLength(p.pts);
        p.start = engine.dist(0, 0, p.pts[0][0], p.pts[0][1]) / speed;
        p.dur = engine.max(160, p.len / speed);
      } else if (p.k === 'blob') {
        p.start = engine.dist(0, 0, p.cx, p.cy) / speed;
        p.dur = 650;
      }
      end = engine.max(end, p.start + p.dur);
    }
    return { x, y, ang, prims, t0: engine.millis() + autoDelay, end };
  }

  function pathLength(pts) {
    let L = 0;
    for (let i = 1; i < pts.length; i++) L += engine.dist(pts[i - 1][0], pts[i - 1][1], pts[i][0], pts[i][1]);
    return L;
  }
  ...
  function easeInOutQuad(q) { return q < 0.5 ? 2 * q * q : 1 - engine.pow(-2 * q + 2, 2) / 2; }
  function smoothstep(a, b, x) { const t = engine.constrain((x - a) / (b - a), 0, 1); return t * t * (3 - 2 * t); }
  ...
  function drawGrowingPath(g, p, q) {
    ...
    const f = prog * (n - 1);
    const i = engine.floor(f);
    const frac = f - i;
    const sub = p.pts.slice(0, i + 1);
    if (i < n - 1) {
      const a = p.pts[i], b = p.pts[i + 1];
      sub.push([engine.lerp(a[0], b[0], frac), engine.lerp(a[1], b[1], frac)]);
    }
    if (sub.length < 2) {
      drawDot(g, p.pts[0][0], p.pts[0][1], p.wAt(0), p.col);
      return;
    }
    const tipD = engine.max(p.wAt(prog) * 1.8, u * 0.7 * bs) * (1 - smoothstep(0.7, 1, q));
    const melt = u * 0.9 * bs;
    const wAt = tt => {
      const t = tt * prog;
      const base = p.wAt(t);
      const dTip = (prog - t) * p.len;
      return engine.max(base, tipD * engine.exp(-engine.pow(dTip / melt, 2)));
    };
  ...
  function drawVarStroke(g, pts, wAt, col) {
    ...
    const left = [], right = [];
    for (let i = 0; i < n; i++) {
      const p0 = pts[engine.max(0, i - 1)], p1 = pts[engine.min(n - 1, i + 1)];
      let tx = p1[0] - p0[0], ty = p1[1] - p0[1];
      const m = engine.sqrt(tx * tx + ty * ty) || 1;
      tx /= m; ty /= m;
      const hw = engine.max(0.35, wAt(i / (n - 1)) / 2);
    ...
    // round the ends
    g.noStroke();
    g.circle(pts[0][0], pts[0][1], engine.max(0.7, wAt(0)));
    g.circle(pts[n - 1][0], pts[n - 1][1], engine.max(0.7, wAt(1)));
  }
  ...
  function autoFill() {
    const base = engine.millis();
    const writeMs = 6000; // how long the page takes to write itself
    const rowH = staffGap() * 9;
    let row = 0;
    for (let y0 = rowH * 0.6; y0 < engine.height; y0 += rowH, row++) {
      let x = engine.random(-u * 4, u * 20), y = y0 + engine.random(-u * 2, u * 2);
      const xEnd = engine.width - engine.random(0, u * 25);
      let a = engine.random() < 0.5 ? 0 : engine.random(-0.6, 0.6);
      const seed = engine.random(1000);
      beginStroke(x, y);
      let i = 0;
      while (x < xEnd && i < 2000) {
        a = engine.lerp(a, (engine.noise(seed + i * 0.02) - 0.5) * 0.9, 0.08);
        x += engine.cos(a) * 6; y += engine.sin(a) * 6;
        autoDelay = (x / engine.width) * writeMs + row * 90 + (engine.millis() - base);
        strokeTo(x, y);
        i++;
      }
      endStroke();
    ...
    for (let s = 0; s < 3; s++) {
      let x = engine.random(engine.width), y = engine.random(engine.height), a = engine.random(engine.TWO_PI);
      const seed = engine.random(1000);
      beginStroke(x, y);
      for (let i = 0; i < 120; i++) {
        a += (engine.noise(seed + i * 0.05) - 0.5) * 0.2;
        x += engine.cos(a) * 6; y += engine.sin(a) * 6;
        if (x < 0 || x > engine.width || y < 0 || y > engine.height) break;
  ...
  function staffGap() { return u * staffSize; }
  function dotD()  { return u * 1.25 * bs; }
  function thinW() { return engine.max(1.2, u * 0.2 * bs); }
  function hairW() { return engine.max(0.9, u * 0.1 * bs); }
  ...
  function bezierPts(x1, y1, cx1, cy1, cx2, cy2, x2, y2, n = 40) {
    return samplePath(t => [
      engine.bezierPoint(x1, cx1, cx2, x2, t),
      engine.bezierPoint(y1, cy1, cy2, y2, t),
    ], n);
  }
  ...
  function meltProfile(w, d1, d2, k = 0.25) {
    return t => {
      let v = w;
      if (d1 > 0) v = engine.max(v, w + (d1 * 0.9 - w) * engine.exp(-engine.pow(t / k, 1.6) * 4));
      if (d2 > 0) v = engine.max(v, w + (d2 * 0.9 - w) * engine.exp(-engine.pow((1 - t) / k, 1.6) * 4));
      return v;
    };
  }
  ...
  function rod(x1, y1, x2, y2, col, d1 = dotD(), d2 = dotD(), w = thinW()) {
    const L = engine.dist(x1, y1, x2, y2);
    if (L < 0.5) return;
    const k = engine.constrain((u * 1.6 * bs) / L, 0.08, 0.5);
    const pts = samplePath(t => [engine.lerp(x1, x2, t), engine.lerp(y1, y2, t)], engine.max(10, engine.floor(L / 2)));
    path(pts, meltProfile(w, d1, d2, k), col);
  }

  // y of staff line k in the local frame
  function ly(L, k) { return (engine.constrain(k, -5, 5) - L.li) * staffGap(); }
  ...
  function snipChain(L) {
    const yy = ly(L, L.li);
    const n = engine.floor(engine.random(2, 7));
    const step = u * engine.random(2.2, 4.2) * bs;
    const multi = engine.random() < 0.3;
    const pts = [];
    for (let k = 0; k < n; k++) pts.push([L.dir * k * step, yy, engine.random() < 0.2 ? dotD() * engine.random(0.6, 0.85) : dotD()]);
    for (let k = 0; k < n - 1; k++) {
    ...
    if (engine.random() < 0.25) {
      const last = pts[n - 1];
      rod(last[0], last[1], last[0] + L.dir * u * engine.random(3, 8) * bs, last[1], L.col, last[2], 0, hairW());
    }
  }

  // Loose dots on nearby lines.
  function snipCluster(L) {
    const n = engine.floor(engine.random(2, 6));
    const multi = engine.random() < 0.5;
    let cx = 0;
    for (let k = 0; k < n; k++) {
      const li = L.li + engine.floor(engine.random(-2, 3));
      const size = engine.random() < 0.15 ? dotD() * 0.55 : dotD();
      dot(cx, ly(L, li), multi ? pickColor() : L.col, size);
      cx += L.dir * u * engine.random(1.6, 3.2) * bs;
    }
  }
  ...
  function snipZigzag(L) {
    let li = L.li;
    const n = engine.floor(engine.random(3, 8));
    const pts = [];
    let cx = 0;
    for (let k = 0; k < n; k++) {
      pts.push([cx, ly(L, li)]);
      li = engine.constrain(li + engine.floor(engine.random(1, 4)) * (engine.random() < 0.5 ? -1 : 1), -5, 5);
      cx += L.dir * u * engine.random(1.5, 3) * bs;
    }
  ...
  function snipSwell(L) {
    const yy = ly(L, L.li);
    const len = u * engine.random(6, 18) * bs;
    const bumps = [];
    const nb = engine.floor(engine.random(2, 6));
    for (let i = 0; i < nb; i++) bumps.push([engine.random(0.05, 0.95), engine.random(0.7, 1.15)]);
    const w = thinW() * engine.random(0.9, 1.6);
    const wAt = t => {
      let v = w;
      for (const [c, s] of bumps) v = engine.max(v, dotD() * s * engine.exp(-engine.pow((t - c) * len / (u * 0.9 * bs), 2)));
      return v;
    };
  ...
  function snipBar(L) {
    const count = engine.random() < 0.35 ? engine.floor(engine.random(2, 4)) : 1;
    for (let k = 0; k < count; k++) {
      const yy = ly(L, L.li + k * 2);
      const len = u * engine.random(3, 12) * bs;
      const w = u * engine.random(1.1, 1.7) * bs;
      const c = k === 0 ? L.col : pickColor();
      const ox = engine.random(-u * 2, u * 2) * bs;
      if (engine.random() < 0.4) {
        // bar that melts into a long thin tail
        const tail = u * engine.random(4, 12) * bs;
        const pts = samplePath(t => [ox + L.dir * t * (len + tail), yy], 60);
        const split = len / (len + tail);
        path(pts, t => t < split ? w : engine.max(hairW(), w * engine.pow(1 - (t - split) / (1 - split), 1.8)), c);
      } else {
  ...
  function snipStem(L) {
    const yy = ly(L, L.li);
    const up = engine.random() < 0.5 ? -1 : 1;
    const len = u * engine.random(3, 8) * bs;
    const ey = yy + up * len;
    const r = engine.random();
    const capD = r < 0.4 ? dotD() * engine.random(0.5, 1) : 0;
    rod(0, yy, 0, ey, L.col, dotD(), capD);
    dot(0, yy, L.col);
    if (capD > 0) dot(0, ey, engine.random() < 0.3 ? pickColor() : L.col, capD);
    else if (r < 0.65) {
      const fl = u * engine.random(1.5, 3) * bs;
  ...
  function snipChord(L) {
    const n = engine.floor(engine.random(2, 5));
    const gap = engine.random() < 0.5 ? 1 : 2;
    const multi = engine.random() < 0.4;
    const top = ly(L, L.li), bottom = ly(L, L.li + (n - 1) * gap);
    if (engine.random() < 0.5) rod(0, top, 0, bottom, L.col);
  ...
  function snipArch(L) {
    const y0 = ly(L, L.li);
    const span = u * engine.random(3, 12) * bs * L.dir;
    const rise = u * engine.random(5, 16) * bs * (engine.random() < 0.7 ? -1 : 1);
    const y1 = y0 + engine.random(-u * 3, u * 3) * bs;
    const pts = bezierPts(0, y0, span * 0.05, y0 + rise * 1.25, span * 0.95, y1 + rise * 1.25, span, y1, 60);
    const d0 = dotD() * engine.random(0.8, 1.1);
    const d1 = engine.random() < 0.7 ? dotD() * engine.random(0.35, 0.7) : 0;
    path(pts, meltProfile(hairW() * engine.random(1, 1.6), d0, d1, 0.2), L.col);
    dot(0, y0, L.col, d0);
  ...
  function snipHook(L) {
    const y0 = ly(L, L.li);
    const down = engine.random() < 0.6 ? 1 : -1;
    const h = u * engine.random(4, 10) * bs;
    const w = u * engine.random(1.5, 4) * bs * L.dir;
    const pts = bezierPts(0, y0, 0, y0 + down * h * 0.9, w * 0.2, y0 + down * h * 1.1, w, y0 + down * h * 0.8, 50);
    const dTip = dotD() * engine.random(0.3, 0.55);
    path(pts, meltProfile(hairW() * engine.random(1, 1.5), dotD(), dTip, 0.3), L.col);
    dot(0, y0, L.col);
  ...
  function snipFan(L) {
    const y0 = ly(L, L.li);
    const n = engine.floor(engine.random(2, 4));
    dot(0, y0, L.col);
    for (let i = 0; i < n; i++) {
      const len = u * engine.random(6, 16) * bs;
      const a = engine.random(-0.9, 0.9) + (L.dir < 0 ? engine.PI : 0);
      const bend = u * engine.random(-5, 5) * bs;
      const ex = engine.cos(a) * len, ey = y0 + engine.sin(a) * len;
      const nx = -engine.sin(a), ny = engine.cos(a);
      const pts = bezierPts(0, y0, ex * 0.4 + nx * bend, y0 + (ey - y0) * 0.4 + ny * bend, ex * 0.75, ey - (ey - y0) * 0.25, ex, ey, 40);
      const d1 = dotD() * engine.random(0.35, 0.7);
      path(pts, meltProfile(hairW(), dotD(), d1, 0.25), L.col);
      dot(ex, ey, engine.random() < 0.3 ? pickColor() : L.col, d1);
    }
  }
  ...
  function snipParens(L) {
    const y0 = ly(L, L.li);
    const h = u * engine.random(4, 12) * bs;
    const gap = u * engine.random(2, 6) * bs;
    const bow = u * engine.random(1, 3) * bs;
    const horizontal = engine.random() < 0.5;
    const w = hairW() * engine.random(1, 1.4);
    const prof = t => w + (w * 1.6) * engine.sin(t * engine.PI);
    for (const s of [-1, 1]) {
      let pts;
      if (horizontal) {
    ...
    if (engine.random() < 0.7) dot(0, y0, engine.random() < 0.4 ? pickColor() : L.col, dotD() * engine.random(0.5, 1));
  }

  // A spiral that unwinds from a dot, thinning as it goes.
  function snipSpiral(L) {
    const y0 = ly(L, L.li);
    const turns = engine.random(1.2, 2.5);
    const rMax = u * engine.random(2, 5) * bs;
    const cw = engine.random() < 0.5 ? 1 : -1;
    const a0 = engine.random(engine.TWO_PI);
    const pts = samplePath(t => {
      const a = a0 + cw * t * turns * engine.TWO_PI;
      const r = rMax * engine.pow(t, 0.8);
      return [engine.cos(a) * r, y0 + engine.sin(a) * r];
    }, 90);
  ...
  function snipCross(L) {
    const y0 = ly(L, L.li);
    const arms = engine.floor(engine.random(3, 5));
    const a0 = engine.random() < 0.6 ? 0 : engine.random(engine.PI);
    dot(0, y0, L.col, dotD() * 1.1);
    for (let i = 0; i < arms; i++) {
      const a = a0 + i * engine.TWO_PI / arms + engine.random(-0.15, 0.15);
      const len = u * engine.random(2.5, 7) * bs;
      const d1 = engine.random() < 0.7 ? dotD() * engine.random(0.5, 1) : 0;
      rod(0, y0, engine.cos(a) * len, y0 + engine.sin(a) * len, L.col, dotD() * 1.1, d1, thinW() * engine.random(0.8, 1.4));
      if (d1 > 0) dot(engine.cos(a) * len, y0 + engine.sin(a) * len, L.col, d1);
    }
  }
  ...
  function snipBlob(L) {
    const y0 = ly(L, L.li);
    const base = u * engine.random(1.4, 3) * bs;
    const stretch = engine.random(1, 2.2);
    const seed = engine.random(1000);
    const rot = engine.random(-0.5, 0.5);
    const steps = 28;
    const pts = [];
    for (let k = 0; k <= steps + 3; k++) {
      const t = (k % steps) / steps * engine.TWO_PI;
      const r = base * (0.7 + 0.6 * engine.noise(seed + engine.cos(t) * 1.3, seed + engine.sin(t) * 1.3));
      const px = engine.cos(t) * r * stretch, py = engine.sin(t) * r;
      pts.push([px * engine.cos(rot) - py * engine.sin(rot), px * engine.sin(rot) + py * engine.cos(rot)]);
    }
    rec.push({ k: 'blob', pts, cx: 0, cy: y0, col: L.col });
    if (engine.random() < 0.45) {
      const a = engine.random(engine.TWO_PI);
      const len = u * engine.random(4, 10) * bs;
      const d1 = dotD() * engine.random(0.4, 0.8);
      rod(0, y0, engine.cos(a) * len, y0 + engine.sin(a) * len, L.col, base * 1.6, d1);
      dot(engine.cos(a) * len, y0 + engine.sin(a) * len, L.col, d1);
    }
  }
  ...
  function snipSwipe(L) {
    const y0 = ly(L, L.li);
    const len = u * engine.random(5, 14) * bs;
    const w = u * engine.random(0.9, 1.6) * bs;
    const a = engine.random(-0.6, 0.6) + (L.dir < 0 ? engine.PI : 0);
    const nx = -engine.sin(a), ny = engine.cos(a);
    const wobble = u * engine.random(-4, 4) * bs;
    const x2 = engine.cos(a) * len, y2 = y0 + engine.sin(a) * len;
    const pts = bezierPts(
      0, y0,
      engine.cos(a) * len * 0.33 + nx * wobble, y0 + engine.sin(a) * len * 0.33 + ny * wobble,
      engine.cos(a) * len * 0.66 - nx * wobble * 0.5, y0 + engine.sin(a) * len * 0.66 - ny * wobble * 0.5,
      x2, y2, 50);
    const taper = engine.random() < 0.6;
    path(pts, t => taper ? engine.max(hairW(), w * engine.pow(1 - t, 1.3)) : w, L.col);
  }
  ...
  function finish() {
    ...
    flushRibbonQueue(Infinity);
    for (const stamp of active) renderSnippet(stamp, paint, Infinity);
    active = [];
  }

  function setSetting(name, value) {
    if (name === 'size') bs = engine.constrain(Number(value), 0.4, 4);
    if (name === 'staffSize') staffSize = engine.constrain(Number(value), 0.25, 4);
    if (name === 'density') density = engine.constrain(Number(value), 0.25, 6);
    if (name === 'color') colorLock = COLOR_KEYS.includes(value) ? value : null;
    if (name === 'staff') showStaff = Boolean(value);
  }
  ...
  return {
    ...
    resize() {
      clearAll();
      for (const layer of [paint, staffLayer]) {
    ...
    clear: clearAll,
    autoFill,
    randomize() { clearAll(); u = engine.floor(engine.random(7, 13)); },
    getSettings() { return { size: bs, staffSize, density, color: colorLock || 'auto', staff: showStaff }; },
    setSetting,
  };
