export default function createBrush(engine) {
  ...
  let inkCol;
  let bs = 0.5;              // cell size multiplier
  ...
  function setup(params = new URLSearchParams()) {
    engine.strokeCap(engine.ROUND);
    engine.strokeJoin(engine.ROUND);
    inkCol = engine.color(...INK);
    paint = makeLayer();
    deal();
  ...
  function draw() {
    ...
    for (const p of pops) {
      ...
      soakBlob(p, engine.drawingContext, now);
    }
    drawLive();
  }
  ...
  function deal() {
    c = CELL * bs;
    const margin = engine.min(engine.width, engine.height) * 0.07;
    cols = engine.max(2, engine.floor((engine.width - 2 * margin) / c));
    rows = engine.max(2, engine.floor((engine.height - 2 * margin) / c));
    ox = (engine.width - cols * c) / 2;
    oy = (engine.height - rows * c) / 2;
    ...
    for (const idx of order) {
      ...
      if (kind === 'block') {
        ...
        for (let tries = 0; tries < 8 && !members; tries++) {
          const x0 = i - engine.floor(engine.random(w)), y0 = j - engine.floor(engine.random(h));
          if (blockFree(x0, y0, w, h)) {
            members = [];
            for (let y = y0; y < y0 + h; y++) for (let x = x0; x < x0 + w; x++) members.push(y * cols + x);
          } else if (engine.random() < 0.5) w = engine.max(1, w - 1);
          else h = engine.max(1, h - 1);
        }
      ...
      if (kind === 'pipe') {
        ...
        const legs = engine.floor(engine.random(1, 4));
        for (let l = 0; l < legs; l++) {
          const n = engine.floor(engine.random(1, 5));
          for (let s = 0; s < n; s++) {
          ...
          dir = engine.random() < 0.5 ? [dir[1], dir[0]] : [-dir[1], -dir[0]];
        }
      }
    ...
    for (const rg of regions) {
      if (rg.members.length !== 1 || engine.random() < 0.3) continue;
    ...
    for (const rg of regions) {
      ...
      rg.jit = [];
      for (let k = 0; k <= 30; k++) rg.jit.push([engine.random(-JITTER, JITTER), engine.random(-JITTER, JITTER)]);
      rg.wash = [];
      for (const idx of rg.members) {
        const i = idx % cols, j = (idx - i) / cols;
        for (let k = 0; k < WASH; k++) {
          rg.wash.push([ox + (i + engine.random()) * c, oy + (j + engine.random()) * c, c * engine.random(0.25, 0.6), engine.random(0.03, 0.08)]);
        }
      }
      ...
      const x0 = engine.min(...xs) - m, y0 = engine.min(...ys) - m, x1 = engine.max(...xs) + m, y1 = engine.max(...ys) + m;
      const n = engine.round(SPECKS * (x1 - x0) * (y1 - y0) / (c * c));
      for (let k = 0; k < n; k++) {
        const x = engine.random(x0, x1), y = engine.random(y0, y1);
        if (nearBlob(rg, x, y, m)) rg.specks.push([x, y, engine.random(0.4, 1.5), engine.random(0.1, 0.4)]);
      }
      rg.revealed = false;
      rg.shown = null;
    }
    ...
    const band = c * BAND;
    layers = engine.constrain(engine.round(band / 1.1), 8, 30);
    for (const key of COLOR_KEYS) {
      rims[key] = [];
      for (let k = 0; k <= layers; k++) {
        const u = k === 0 ? 0 : (k - 1) / (layers - 1);
        const mix = k === 0 ? 0 : 0.3 + 0.7 * (1 - engine.pow(1 - u, 1.8));
  ...
  function nearBlob(rg, x, y, m) {
    for (const idx of rg.members) {
      ...
      const dx = engine.max(cx - x, 0, x - cx - c), dy = engine.max(cy - y, 0, y - cy - c);
  ...
  function regionAt(x, y) {
    const i = engine.floor((x - ox) / c), j = engine.floor((y - oy) / c);
  ...
  function roundedPath(loops, R) {
    ...
    for (const L of loops) {
      ...
      for (let k = 0; k < n; k++) {
        ...
        const r = engine.min(R, engine.dist(a[0], a[1], v[0], v[1]) / 2, engine.dist(v[0], v[1], b[0], b[1]) / 2);
  ...
  function soakBlob(pop, ctx, now) {
    ...
    const dry = 1 - engine.pow(1 - engine.constrain((t - BURST) / DRY, 0, 1), 2);
    const spread = 1 + WET * (1 - dry);
    const fade = 1 - engine.pow(1 - engine.constrain(t / BURST, 0, 1), 3);
    ...
    let wetImage = null;
    if (dry < 1) {
      const off = pop.off, pd = engine.pixelDensity();
    ...
    if (dry > 0 && !pop.dryImage) {
      const off = pop.off.cloneNode(), pd = engine.pixelDensity();
      const octx = off.getContext('2d');
      octx.setTransform(pd, 0, 0, pd, -pop.bx * pd, -pop.by * pd);
  ...
  function advanceStroke() {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      if (d < 0.4) break;
      const step = engine.min(d, engine.max(1.5, d * 0.18));
  ...
  function strokeTo(x, y) {
    ...
    if (engine.dist(last[0], last[1], x, y) >= 2) st.raw.push([x, y]);
  }
  ...
  function endStroke() {
    ...
    addLine(resample(pts, RES), engine.millis());
  }
  ...
  function addLine(pts, t0) {
    ...
    for (const [x, y] of pts) {
      ...
      pop.bx = engine.min(...xs) - pad; pop.by = engine.min(...ys) - pad;
      pop.bw = engine.max(...xs) - engine.min(...xs) + pad * 2; pop.bh = engine.max(...ys) - engine.min(...ys) + pad * 2;
      pop.off = document.createElement('canvas');
      pop.off.width = engine.ceil(pop.bw * engine.pixelDensity());
      pop.off.height = engine.ceil(pop.bh * engine.pixelDensity());
    }
  }
  ...
  function drawLive() {
    if (!st) return;
    const pts = st.raw;
    if (pts.length < 2) { engine.noStroke(); engine.fill(inkCol); engine.circle(pts[0][0], pts[0][1], LINE_W * 2); return; }
    engine.noFill();
    engine.stroke(inkCol);
    engine.strokeWeight(LINE_W);
    engine.beginShape();
    for (const p of pts) engine.vertex(p[0], p[1]);
    engine.endShape();
  }
  ...
  function resample(pts, step) {
    ...
    for (let i = 1; i < pts.length; i++) {
      ...
      let d = engine.dist(ax, ay, bx, by);
      while (d > 0 && carry + d >= step) {
        const t = (step - carry) / d;
        ax += (bx - ax) * t; ay += (by - ay) * t;
        out.push([ax, ay]);
        d = engine.dist(ax, ay, bx, by);
        carry = 0;
      }
      carry += d;
    }
    const last = pts[pts.length - 1], end = out[out.length - 1];
    if (out.length === 1 || engine.dist(last[0], last[1], end[0], end[1]) > 0.3) out.push(last.slice());
    return out;
  }
  ...
  function autoFill() {
    const count = engine.round((cols * rows) / 60) + 2;
    let t0 = engine.millis() + 300;
  ...
  function randomCurve() {
    ...
    let x = engine.random(x0, x1), y = engine.random(y0, y1);
    let h = engine.random(engine.TWO_PI);
    const seed = engine.random(1000);
    const n = engine.floor(engine.random(40, 160));
    const pts = [[x, y]];
    for (let i = 0; i < n; i++) {
      h += (engine.noise(seed + i * 0.06) - 0.5) * 0.5;
      x += engine.cos(h) * 4; y += engine.sin(h) * 4;
      if (x < x0 || x > x1 || y < y0 || y > y1) {
        h += engine.HALF_PI;
        x = engine.constrain(x, x0, x1); y = engine.constrain(y, y0, y1);
      }
      pts.push([x, y]);
    }
    return chaikin(pts);
  ...
  function css(rgb) { return `rgb(${rgb.map(v => engine.round(v)).join(',')})`; }
  function pick(a) { return a[engine.floor(engine.random(a.length))]; }
  function shuffle(a) {
    for (let i = a.length - 1; i > 0; i--) { const j = engine.floor(engine.random(i + 1)); [a[i], a[j]] = [a[j], a[i]]; }
    return a;
  }
  ...
  function finish() {
    ...
    for (const pop of pops) paintBlob(pop.rg, paint.drawingContext, pop.col, 1);
    pops = [];
  }

  function setSetting(name, value) {
    if (name === 'size') { bs = engine.constrain(Number(value), 0.4, 3); deal(); }
  ...
  return {
    ...
    autoFill,
    randomize() { deal(); },
    getSettings() { return { size: bs, color: colorLock || 'auto', revealed: regions.filter(rg => rg.revealed).length, total: regions.length }; },
    setSetting,
  };
